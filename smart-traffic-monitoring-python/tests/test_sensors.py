...
def test_create_sensor(client: TestClient, db_session: Session, example_sensor_data):
    ...
    # 3. Create a sensor
    response = client.post("/sensors/", json=example_sensor_data)
...
def test_get_all_sensors(client: TestClient, db_session: Session, example_sensor_data):
    # 1. Create a station
    station_data = {
        "code": f"STA-{uuid.uuid4().hex[:6].upper()}",
    ...
    response1 = client.post("/sensors/", json=sensor_1)
    assert response1.status_code == 200, f"Failed to create sensor 1: {response1.text}"
    response2 = client.post("/sensors/", json=sensor_2)
...
def test_update_sensor(client: TestClient, db_session: Session, example_sensor_data):
    # 1. Create a station
    station_data = {
        "code": f"STA-{uuid.uuid4().hex[:6].upper()}",
    ...
    update_resp = client.put(f"/sensors/{sensor_id}", json=update_data)
...
def test_delete_sensor(client: TestClient, db_session: Session, example_sensor_data):
    # 1. Create a station
    station_data = {
        "code": f"STA-{uuid.uuid4().hex[:6].upper()}",
    ...
    get_resp = client.get(f"/sensors/{sensor_id}")
