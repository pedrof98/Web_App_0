...
func SetupDatabase() *gorm.DB {
	...
	for i := 0; i < 10; i++ {
		db, err = gorm.Open(postgres.Open(dsn), &gorm.Config{
			Logger:                                   logger.Default.LogMode(logger.Silent), // Set silent to avoid cluttering logs - switch to info for debug
			DisableForeignKeyConstraintWhenMigrating: true,
			PrepareStmt:                              true,
			CreateBatchSize:                          1000, // Adjust batch size as needed
